...
// Base URLs for TradeLockers API
const TRADELOCKERS_BASE_URL = "https://api.tradelockers.com/v1"

// Function to get API keys from localStorage or sessionStorage
function getApiKeys() {
  const apiKey = localStorage.getItem("tradelockersApiKey") || sessionStorage.getItem("tradelockersApiKey")
  const apiSecret = localStorage.getItem("tradelockersApiSecret") || sessionStorage.getItem("tradelockersApiSecret")
  const isPaper = localStorage.getItem("tradelockersPaper") !== "false" // Default to paper trading

  // For demo purposes, if keys aren't found, use mock keys instead of throwing an error
  if (!apiKey || !apiSecret) {
    console.warn("TradeLockers API keys not found. Using demo mode with mock data.")
    return {
      apiKey: "DEMO_API_KEY",
...
export async function getAccountInfo() {
  try {
    ...
    // In a real implementation, you would make an API call to TradeLockers
    const response = await axios.get(`${TRADELOCKERS_BASE_URL}/account`, {
      headers: {
...
export async function getPositions() {
  try {
    ...
    const response = await axios.get(`${TRADELOCKERS_BASE_URL}/positions`, {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "Secret-Key": apiSecret,
      },
...
export async function getOrders() {
  try {
    ...
    const response = await axios.get(`${TRADELOCKERS_BASE_URL}/orders?status=all&limit=20`, {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "Secret-Key": apiSecret,
      },
...
export async function executeOrder(orderData: any) {
  try {
    ...
    const response = await axios.post(`${TRADELOCKERS_BASE_URL}/orders`, orderData, {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "Secret-Key": apiSecret,
...
export async function closePosition(symbol: string) {
  try {
    ...
    const response = await axios.delete(`${TRADELOCKERS_BASE_URL}/positions/${symbol}`, {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "Secret-Key": apiSecret,
      },
...
export async function getAccountActivities() {
  try {
    ...
    const response = await axios.get(`${TRADELOCKERS_BASE_URL}/account/activities`, {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "Secret-Key": apiSecret,
      },
...
export async function getMarketData(symbol: string) {
  try {
    ...
    const response = await axios.get(`${TRADELOCKERS_BASE_URL}/market/data/${symbol}`, {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "Secret-Key": apiSecret,
      },
